...
const EditProfile2 = ({ token, isOpen, onClose, onSave }) => {
...
const ProfileAndSkills = ({ profile, onUpdateProfile, token }) => {
  ...
  >
    <Box
      className="relative shadow-lg rounded-lg w-full max-w-4xl mx-auto flex flex-col md:flex-row bg-white"
    >
      {/* Left Side Box */}
      <Box className="w-full md:w-1/3 h-full p-6 flex flex-col items-center justify-between border-r border-gray-200">
        <Avatar size="2xl" name={profile.username} src={profile.profilePicture} />
        <Divider w="100%" />
        <VStack align="center" spacing={2} className="mt-auto">
      ...
      <Box className="w-full md:w-2/3 h-full flex flex-col">
        ...
        <Box className="h-1/2 p-6 flex flex-col items-center justify-center">
          <Heading as="h2" size="lg" textAlign="center">
            {profile.username}
          </Heading>
          <Divider w="100%" />
          <VStack align="center" spacing={2} className="mt-4">
            <Text fontSize="sm" color="gray.500">
              Email
            </Text>
            <Text fontSize="md" color="gray.700">
